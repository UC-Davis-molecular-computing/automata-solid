...
import { assert, GRAPHVIZ_NODE_FONT_SIZE, GRAPHVIZ_NODE_MARGIN, GRAPHVIZ_NODE_WIDTH, GRAPHVIZ_NODE_HEIGHT } from '../../core/Utils'
...
export const NFAComponent: Component<NFAComponentProps> = (props) => {
  ...
  // Get stateSetsVisited from appState computation - only call when hasNavigationData() is true  
  ...
  const generateDotGraph = () => {
    ...
    props.nfa.states.forEach(stateName => {
      ...
      const isCurrent = hasExecutionData() && currentStateSet.includes(stateName)
    ...
    props.nfa.states.forEach(fromState => {
      ...
      props.nfa.inputAlphabet.forEach(symbol => {
        ...
        if (toStates && toStates.length > 0) {
          const isCurrentTransition = hasExecutionData() &&
